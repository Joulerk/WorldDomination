...
<div class="container mt-5">
    <h1 class="text-center display-4 mb-4">Присоединение к комнате</h1>
    <div class="card shadow-sm mt-5">
        <div class="card-body">
            <form action="index.php" method="POST">
                ...
                <button type="submit" name="join_room" class="btn btn-success mt-3">Присоединиться</button>
            </form>
            <form action="index.php" method="POST" class="mt-3">
                <button type="submit" name="create_room" class="btn btn-primary mt-3">Создать новую комнату</button>
            </form>
            <form action="manage_round.php" method="GET" class="mt-3">
                <label for="room_manage">Название комнаты:</label>
                <select id="room_manage" name="room" class="form-control" required>
                    <?php foreach ($rooms_data as $room): ?>
                        <option value="<?php echo htmlspecialchars($room['name']); ?>"><?php echo htmlspecialchars($room['name']); ?></option>
                    <?php endforeach; ?>
                </select>
                <button type="submit" class="btn btn-warning mt-3">Управлять раундом</button>
            </form>
            <?php if (!empty($deletion_timers)): ?>
                <div class="mt-3">
                    <h4>Таймеры удаления комнат:</h4>
